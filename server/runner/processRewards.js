        ...
        if (!results.length) {
          ...
          if (currentCustomer[0].phone_number || isCustomerEmailValid) {
            ...
            const ruleCalc = rules.reduce((prev, curr) => {
              const ruleAssessment = transactionsFiltered[i][curr.rule_field] >= curr.rule_assertion_value
              if (ruleAssessment) {
                prev.points_awarded += Math.round(transactionsFiltered[i][curr.rule_field] / curr.rule_assertion_value * curr.points_required)
            ...
            if (ruleCalc.pass) {
              ...
              await db('reward_actions')
                .insert({
                  action_id: transactionsFiltered[i].uniq_id,
                  updated_by: 'sys',
                  members_src_id: transactionsFiltered[i].customer_id,
